...
Parser.prototype = {
   ...
   {
      let context = this.getParseContext();
      context.targetContainer.detailDescription = context.markdown.text;
   },
   ...
   {
      let context = this.getParseContext();
      context.targetContainer.inbodyDescription = context.markdown.text;
   },
   ...
   endParseVerbatim: function (node)
   {
      let context = this.getParseContext();
      context.markdown.text += "\n";
      context.markdown.text = markdownEngine.render(context.markdown.text.replace(/^\*\s{0,1}$/gm, '').replace(/^\*\s/gm, ''));
   },
   ...
   beginParseSimplesect: function (node)
   {
      ...
      if (attrs.kind == "return") {
         if (!context.generateReturnHeader) {
            context.markdown.text += '<dl class = "fn-return"><dt>返回值</dt>';
            context.generateReturnHeader = true;
         }
   ...
   endParseSimplesect: function (tagName)
   {
      ...
      if (attrs.kind == "return") {
         let parentContext = context.parentContext;
         if (!parentContext.generateReturnHeader) {
