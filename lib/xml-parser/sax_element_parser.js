...
Parser.prototype = {
   ...
   {
      let context = this.getParseContext();
      context.targetContainer.detailDescription = markdownEngine.render(_.trim(context.markdown.text));
      //console.log(context.targetContainer.detailDescription)
   },
   ...
   beginParseHeading: function (node)
   {
      let context = this.getParseContext();
      let attrs = node.attributes;
      let level = attrs.level;
      context.markdown.text += "\n"+_.repeat('#',level);//单独一个段落
      this.parseContextStack.push({
         node: node,
         tagName: node.name,
         markdown: context.markdown
   ...
   notifyHeadingText: function (text)
   {
      let context = this.getParseContext();
      context.markdown.text += " "+text;
   },

   endParseHeading: function (tagName)
   {
      let context = this.getParseContext();
      context.markdown.text += "\n";//单独一个段落
   },

   beginParseHruler: function (node)
   {
      let context = this.getParseContext();
      context.markdown.text += "\n<hr />\n";
   },

   beginParseNdash: function (node)
   {
      let context = this.getParseContext();
      context.markdown.text += "&ndash;";
   },

   beginParseMdash: function (node)
   {
      let context = this.getParseContext();
      context.markdown.text += "&mdash;";
   },

   beginParseBold:function (node)
   {
      let context = this.getParseContext();
      context.markdown.text += "**";
      this.parseContextStack.push({
   ...
   notifyBoldText: function (text)
   {
      let context = this.getParseContext();
      context.markdown.text += text;
   },

   endParseBold: function (tagName)
   {
      let context = this.getParseContext();
      context.markdown.text += "**";
   },

   beginParseEmphasis:function (node)
   {
      let context = this.getParseContext();
      context.markdown.text += "*";
      this.parseContextStack.push({
   ...
   notifyEmphasisText: function (text)
   {
      let context = this.getParseContext();
      context.markdown.text += text;
   },

   endParseEmphasis: function (tagName)
   {
      let context = this.getParseContext();
      context.markdown.text += "*";
   },

   beginParseBlockquote:function (node)
   {
      let context = this.getParseContext();
      if (!this.persistDataPool.has("BlockquoteLevel")) {
         this.persistDataPool.set("BlockquoteLevel", 1);
      }
      let level =  this.persistDataPool.get("BlockquoteLevel");
      context.markdown.text += "\n"+_.repeat('> ', level);
      this.parseContextStack.push({
         node: node,
         tagName: node.name,
      ...
      this.persistDataPool.set("BlockquoteLevel", ++level);
   },

   notifyBlockquoteText: function (text)
   {
      let context = this.getParseContext();
      context.markdown.text += text;
   },

   endParseBlockquote: function (tagName)
   {
      let context = this.getParseContext();
      context.markdown.text += "\n";
      if (this.persistDataPool.has("BlockquoteLevel")) {
   ...
   beginParseComputeroutput: function (node)
   {
      let context = this.getParseContext();
      context.markdown.text += "`";
      this.parseContextStack.push({
   ...
   notifyComputeroutputText: function (text)
   {
      let context = this.getParseContext();
      context.markdown.text += text;
   },

   endParseComputeroutput: function (tagName)
   {
      let context = this.getParseContext();
      context.markdown.text += "`";
   },
   ...
   {
      let context = this.getParseContext();
      if (!this.persistDataPool.has("ItemizedlistLevel")) {
         this.persistDataPool.set("ItemizedlistLevel", 0);
      }
      let level = this.persistDataPool.get("ItemizedlistLevel");
      this.persistDataPool.set("ItemizedlistLevel", ++level);
      this.parseContextStack.push({
         node: node,
         tagName: node.name,
         markdown: context.markdown,
