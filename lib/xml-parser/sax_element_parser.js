...
Parser.prototype = {
   ...
   endParseMemberdef: function (tagName)
   {
      ...
      if (menberKind == "function") {
         ...
         let name = Parser.REF_TPL.replace("{url}", Utils.url_for_entity_detail(this.hexo, func.containerId, func.id)).
   ...
   beginParseRef: function(node)
   {
      ...
      // 检查上下文
      if (tagName != "type" && tagName != "defval") {
         return;
      }
      let attrs = node.attributes;
      let ref = {
   ...
   endParseRef: function (tagName)
   {
      ...
      let refStr = Parser.REF_TPL.replace("{url}", Utils.url_for_api_entity(this.hexo, targetContainer.refid)).
