...
function Parser(filename, container, register)
{
...
Parser.prototype = {
   ...
   beginParseType: function (node)
   {
      let context = this.getParseContext();
      let types = [];
      context.targetContainer.types = types;
      this.parseContextStack.push({
         node: node,
         tagName: node.name,
         targetContainer: types
      });
   },
   ...
   {
      let context = this.getParseContext();
      context.targetContainer.push(text);
   },
   ...
   beginParseRef: function(node)
   {
      ...
      context.targetContainer.push(ref);
      this.parseContextStack.push({
         node: node,
         tagName: node.name,
         targetContainer: ref
