...
Parser.prototype = {
   ...
   {
      let context = this.getParseContext();
      context.targetContainer.briefDescription = markdownEngine.render(_.trim(context.markdown.text));
   },
   ...
   {
      let context = this.getParseContext();
      context.targetContainer.detailDescription = _.trim(context.markdown.text);
   },
   ...
   {
      let context = this.getParseContext();
      context.targetContainer.inbodyDescription = markdownEngine.render(_.trim(context.markdown.text));
   ...
   endParseVerbatim: function (tagName)
   {
      let context = this.getParseContext();
      context.markdown.text = context.markdown.text.replace(/^\*\s{0,1}$/gm, '').replace(/^\*\s/gm, '');
      context.markdown.text = markdownEngine.render(context.markdown.text);
   },
   ...
   {
      let context = this.getParseContext();
      if (!this.persistDataPool.has("ListLevel")) {
         this.persistDataPool.set("ListLevel", 0);
      }
      if (!this.persistDataPool.has("ListTypeStack")) {
         this.persistDataPool.set("ListTypeStack", []);
      }
      let level = this.persistDataPool.get("ListLevel");
      let typeStack = this.persistDataPool.get("ListTypeStack");
      typeStack.push("unorder");
      this.persistDataPool.set("ListLevel", ++level);
      this.parseContextStack.push({
         node: node,
         tagName: node.name,
         markdown: context.markdown,
   ...
   notifyItemizedlistText: function (text)
   {
      let context = this.getParseContext();
      context.markdown.text += text;
   },
   ...
   endParseItemizedlist: function (tagName)
   {
      ...
      if (this.persistDataPool.has("ListTypeStack")) {
         let typeStack = this.persistDataPool.get("ListTypeStack");
         typeStack.pop();
         this.persistDataPool.set("ListTypeStack", typeStack);
      }
   },
   ...
   beginParseListitem: function (node)
   {
      ...
      let listItemPre = "+ "; // 默认unorder
      if (type == "unorder") {
         listItemPre = "+ "
      } else if (type == "order") {
         listItemPre = "1. "
      }
      context.markdown.text += "\n"+_.repeat('\t', level - 1)+listItemPre;
      this.parseContextStack.push({
         node: node,
         tagName: node.name,
         markdown: context.markdown,
   ...
   // },
   //
   // notifyOrderedlistText: function (text)
   // {
   //    let context = this.getParseContext();
   //    context.markdown.text += text;
   // },
   ...
   // },
   //
   // beginParseProgramlisting: function (node)
   // {
   //    let context = this.getParseContext();
   //    if (context.tagName == "compounddef") {
   //       context.markdown = {
   //          text: ""
   //       }; // 不正常状态 doxygen自身bug
   //    }
   //    context.markdown.text += "\n```";
   //    if (!this.persistDataPool.has("InCodeBlock")) {
   //       this.persistDataPool.set("InCodeBlock", true);
   //    }
   //    this.parseContextStack.push({
   //       node: node,
   //       tagName: node.name,
   ...
   // },
   //
   // notifyProgramlistingText: function (text)
   // {
   //    let context = this.getParseContext();
   //    //context.markdown.text += text;
   // },
   //
   // endParseProgramlisting: function (tagName)
   // {
   //    let context = this.getParseContext();
   //    context.markdown.text += "```\n\n";
   //    if (this.persistDataPool.has("InCodeBlock")) {
   //       this.persistDataPool.set("InCodeBlock", false);
   //    }
   // },
   //
   // beginParseCodeline: function (node)
   // {
   //    let context = this.getParseContext();
   //    this.parseContextStack.push({
   //       node: node,
   ...
   // },
   //
   // notifyCodelineText: function (text)
   // {
   //    let context = this.getParseContext();
   //    context.markdown.text += text;
   // },
   //
   // beginParseSp: function (node)
   // {
   //    let context = this.getParseContext();
   //    context.markdown.text += " ";
   // },
   //
   // beginParseHighlight: function (node)
   // {
   //    let context = this.getParseContext();
   //    this.parseContextStack.push({
   //       node: node,
   ...
   // },
   //
   // notifyHighlightText: function (text)
   // {
   //    let context = this.getParseContext();
   //    context.markdown.text += text;
   // },
   ...
   // {
   //    let context = this.getParseContext();
   //    context.markdown.text += "<table><tbody>";
   //    this.parseContextStack.push({
   //       node: node,
   //       tagName: node.name,
   //       markdown: context.markdown
   ...
   // },
   //
   // endParseTable: function (tagName)
   // {
   //    let context = this.getParseContext();
   //    context.markdown.text += "</tbody></table>";
   // },
   //
   ...
   // endParseEntry: function (tagName)
   // {
   //    let context = this.getParseContext();
   //    let attrs = context.node.attributes;
   //    if (attrs.thead == "yes") {
