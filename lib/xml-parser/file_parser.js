...
module.exports = {
   ...
   post_parse_hook(context)
   {
      ...
      let funcs = {};
      _.values(context.doxygen.files).map(function(file){
         file.funcs.map(function(func){
            if (func.argsstring.indexOf("->") != -1) {
               let containerId = func.containerId;
               if (!funcs[containerId]) {
                  funcs[containerId] = [];
               }
               funcs[containerId].push(func);
            }
         });
      });
      return Promise.all(Utils.fix_funcs_signature(funcs, context));
   }
};